...
reseni = [solve_ivp(
                   lambda t,X:konkurence(t,X,*args),
                   meze,
                   pocatecni_podminka,
                   t_eval=t
                   )
        for args in [(1,1,0.4,1,0.2,1),(1,1,0,1,0,1)] ]

ax.plot(reseni[0].t,reseni[0].y.T,label=[r"Druh $x$ v konkurenci s $y$", r"Druh $y$ v konkurenci s $x$"])
ax.plot(reseni[1].t,reseni[1].y.T,"--",label=[r"Druh $x$ samostatně", r"Druh $y$ samostatně"])
ax.set(
    ylim=(0,1.1),
    title="Model dvou populací s konkurencí a bez konkurence",
    xlabel="Čas",
    ylabel="Bezrozměrná velikost")
plt.legend();
```
...
Na obrázku je schematicky zachycena jedna z takových situací. V průmětnách jsou
