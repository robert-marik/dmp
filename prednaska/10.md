...
```{code-cell} ipython
:tags: ["hide-input"]
def predator(x,y,c=0.04,d=.1, m=4):
    return -c*y + d*x*y/(m+x)

def prey(x,y,r=1,b=0.3,K=10, m=4):
    return r*(1-x/K)*x-b*x*y/(4+x)
...
fig,ax = plt.subplots()
kmin = df.xs(key="kořist",level=1,axis=1).min().min()
kmax = df.xs(key="kořist",level=1,axis=1).max().max()
dmin = df.xs(key="dravec",level=1,axis=1).min().min()
dmax = df.xs(key="dravec",level=1,axis=1).max().max()
ax.set(
    xlabel="velikost populace kořisti",
    ylabel="velikost populace dravce",
    title="Rosenzweigův a MacArthurův model dravce a kořisti",
